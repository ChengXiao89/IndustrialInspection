...
{
    std::this_thread::sleep_for(std::chrono::milliseconds(3000));//为了等MV_CC_GetImageBuffer调用后再发送软触发命令
...
int main()
{
    ...
    do
    {
        ...
        if (false && stDeviceList.pDeviceInfo[nIndex]->nTLayerType == MV_GIGE_DEVICE)
        {
        ...
        unsigned int nGrabStrategy = MV_GrabStrategy_LatestImagesOnly;
        //设置GrabStrategy
        if(1)
        {

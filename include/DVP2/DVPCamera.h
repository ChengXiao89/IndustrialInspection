...
#endif
#else
#include <windows.h>        //不能在它前面使用 using namespace std;
#include <stdint.h>
#ifndef __cplusplus
...
typedef enum dvpEvent
{
	...
	/** @brief GPIO，“外部输入行频超出最大行频接收能力” 异常提示 */
	EVENT_LINE_RATE_OVERLOAD = 25,

    /** @brief 设备端发生了丢帧(数据源带宽大于后端传输带宽，buffer满了导致) */
    EVENT_DEV_FRAME_DROP=26,

    /** @brief 驱动端发生了帧覆盖(用户取帧频率低于采集帧率或ISP处理不过来导致) */
...
*@see dvpGetFramesPerTrigger */
    ...
    /** @brief Line的电平状态，每个bit对应一个Line，bit0对应Line1 */
...
/**
*@brief 锁住指定Buffer指针,ReleaseFrameBuffer将不退还指针到空闲队列
*@param[in] handle 相机句柄
*@param[in] pFrameBuffer Buffer指针
*/
dvp2api dvpStatus dvpLockFrameBuffer(IN dvpHandle handle, void* pFrameBuffer);

/**
*@brief 解锁指定Buffer指针，解锁后退还到空闲队列
*@param[in] handle 相机句柄
*@param[in] pFrameBuffer Buffer指针
*/
...
/**
*@brief 格式转换
*@param[in] pFrameIn 输入的帧信息
*@param[in] pBufferIn 输入的buffer
*@param[in,out] pFrameOut 输出的帧信息
*@param[in,out] pBufferOut 输出的buffer
*/
